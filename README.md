...
下载并保存单个网页是通过`save_html()`函数实现的，这个函数提供了两种不同的下载方法：使用`get_html()`下载函数（下面详细描述），或者系统自带模块`urllib.request`。使用时向`save_html()`函数传递网页的URL、保存的文件名和类别，然后函数就会自动按类别创建文件夹，然后将URL指向的网页按指定的文件名保存到这个文件夹里。
